...
namespace Sandswept.Utils
{
    public class BeizerCurve
    {
        ...
        public BeizerCurve(Vector3 p0, Vector3 p1, Vector3 p2, int subdivisions = 200) {
            this.p0 = p0;
            this.p1 = p1;
            this.p2 = p2;
        ...
        public Vector3 GetRotationAlongCurve(float time, float quality = 0.01f) {
            Vector3 p1 = GetBeizerPoint(time);
            Vector3 p2 = GetBeizerPoint(time + quality);

            return (p2 - p1).normalized;
        }

        public Vector3 GetRotationAlongCurveAtDistance(float distance, float quality = 0.22f) {
